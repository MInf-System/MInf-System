			...
			<scroll-view class="scroll-view_H" scroll-x="true" @scroll="scroll" scroll-left="120">
				<view class="uni-container">
					...
					<view class="uni-pagination-box">
						<uni-pagination prev-text="前一页" next-text="后一页" show-icon :page-size="pageSize" :current="pageCurrent" :total="total" @change="change" />
						<view class="btn-view">
							<view>
	...
	export default {
		...
		methods: {
			...
			async totarData(){
				const userTable = await Kvite.buildDefaultKvite('testDb', 'user');
				//为totar赋值
				this.total = await userTable.size();
			...
			async getData(pageCurrent, value = '') {
				...
				//前面的行数
				const current = (pageCurrent - 1) * this.pageSize ;
				
				this.loading = true;

				if(value.length == 0){
					console.log("非搜索查询");
					console.log("前面行数", current);
					const res = await userTable.selectSQL(
						`SELECT key, value FROM ${tableName} LIMIT ${this.pageSize} OFFSET ${current}`
					);
					console.log("查询结束");
					const pagingData = new Array(res.length);
					console.log("开始序列化");
